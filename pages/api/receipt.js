...
export default async (req, res) => {
    ...
    items.forEach((item) => {
      itemsHTML += `
        <tr>
          ...
          <td>
            <div class="item-quantity">${item.size}</div>
          </td>
          <td>
  ...
  const htmlTemplate = `
    ...
    <body>
      ...
      <div class="header">
        ...
        <div class="total">
            <p><strong>Subtotal:</strong> $${orderData.itemsPrice.toFixed(
              2
            )}</p>
            <p><strong>Shipping Fee:</strong> $${orderData.shippingPrice.toFixed(
              2
            )}</p>
  ...
  try {
    ...
    const emailRes = await transporter.sendMail({
      from: '"Dragon Organics" <dragonorganics.tm@gmail.com>',
      to: orderData.shippingAddress.email,
      subject: `Dragon Organics Receipt`,
      html: htmlTemplate,
    });
